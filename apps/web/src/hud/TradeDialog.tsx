...
export function TradeDialog({ onClose }: { onClose: () => void }) {
  ...
  const List = ({ items, chosen, onToggle }: { items: { s: { mortgaged: boolean }; i: number }[]; chosen: number[]; onToggle: (i: number) => void }) => (
    <ul className="space-y-1 max-h-44 sm:max-h-56 overflow-auto pr-1">
      {items.length === 0 && <li className="text-slate-500 text-xs">No properties</li>}
      {items.map(({ s, i }) => {
        const blocked = hasBuildings(i);
        return (
          <li key={i}>
            <label className={`flex items-center gap-2 text-sm py-1 sm:py-0 ${blocked ? "opacity-40" : ""}`}>
              <input type="checkbox" className="w-4 h-4" disabled={blocked} checked={chosen.includes(i)} onChange={() => onToggle(i)} />
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-30 flex items-end sm:items-center justify-center overflow-y-auto overscroll-contain" onClick={onClose}>
      <div
        className="bg-slate-900 border border-slate-700 w-full sm:w-[640px] sm:max-w-[95vw] rounded-t-2xl sm:rounded-xl p-4 max-h-[92dvh] sm:max-h-[90vh] overflow-y-auto pad-safe-bottom"
      ...
      >
        <div className="flex flex-wrap items-center gap-3 mb-3">
          ...
          <label className="text-sm flex items-center gap-2">
            With
            <select className="bg-slate-800 rounded px-2 py-1.5 text-base sm:text-sm" value={toId} onChange={(e) => { setToId(e.target.value); setGetProps([]); }}>
              {others.map((p) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 text-sm">
              <span>Cash</span>
              <input type="number" min={0} max={meP.cash} className="bg-slate-800 rounded px-2 py-1.5 w-24 text-base sm:text-sm" value={giveCash} onChange={(e) => setGiveCash(Math.max(0, Math.min(meP.cash, +e.target.value)))} />
              {meP.jailCards.length > 0 && (
                <>
                  <span className="ml-2">Jail cards</span>
                  <input type="number" min={0} max={meP.jailCards.length} className="bg-slate-800 rounded px-2 py-1.5 w-16 text-base sm:text-sm" value={giveCards} onChange={(e) => setGiveCards(+e.target.value)} />
                </>
              )}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 text-sm">
              <span>Cash</span>
              <input type="number" min={0} max={themP?.cash ?? 0} className="bg-slate-800 rounded px-2 py-1.5 w-24 text-base sm:text-sm" value={getCash} onChange={(e) => setGetCash(Math.max(0, +e.target.value))} />
              {(themP?.jailCards.length ?? 0) > 0 && (
                <>
                  <span className="ml-2">Jail cards</span>
                  <input type="number" min={0} max={themP?.jailCards.length} className="bg-slate-800 rounded px-2 py-1.5 w-16 text-base sm:text-sm" value={getCards} onChange={(e) => setGetCards(+e.target.value)} />
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-end gap-2 mt-3 sticky bottom-0 bg-slate-900 pt-2">
